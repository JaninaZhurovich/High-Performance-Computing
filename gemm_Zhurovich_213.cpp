...
void gemmSerialVerifier(const double *A, const double *B, double *C, int n) {
    for(int i = 0; i < n; ++i) {
        for(int j = 0; j < n; ++j) {
            for (int k = 0; k < n; ++k) {
...
bool verifyResults(const double *A, const double *B, const double *C, int n, const double accuracy = 0.001)
{
    double *correctResultMatrix = new double[n*n];
    errorChk(correctResultMatrix, "new failed!");

    /*
     * We assume that the most simple serial gemm implementation is correct.
     */
    gemmSerialVerifier(A, B, correctResultMatrix, n);
...
int main(int argc, char *argv[])
{
    ...
    if (!verifyResults(A, B, C, matrixSize)) {
